...
def main():
    ...
    parser.add_argument("-m", "--interpolation", dest="interpolate",
                        help="specify the interpolation method (nearest_neighbor, bilinear, or cubic)", metavar="INTERPOLATION METHOD")
    ...
    if args.interpolate is None:
        ...
        if args.interpolate not in ["nearest_neighbor", "bilinear", "cubic"]:
    ...
    resampled_image = resample_obj.resize(input_image, scalex=fx, scaley=fy, interpolation=interpolation)
