...
def main():
    ...
    output_image_name = outputDir + "binary_image_" + datetime.now().strftime("%m%d-%H%M%S") + ".png"
    cv2.imwrite(output_image_name, binary_img, [cv2.IMWRITE_PNG_COMPRESSION, 0])
    ...
    output_image_name = outputDir + "cell_stats_" + datetime.now().strftime("%m%d-%H%M%S") + ".png"
    cv2.imwrite(output_image_name, cell_stats_img, [cv2.IMWRITE_PNG_COMPRESSION, 0])
